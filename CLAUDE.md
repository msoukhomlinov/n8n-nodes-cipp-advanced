...
**Status:** v1.1.3 Beta. Some operations depend on a custom CIPP-API fork (notably Teams Shift and ExecGraphRequest).
...
├── credentials/
│   └── CippAdvancedApi.credentials.ts # Azure AD client credentials + tenant cache settings
├── nodes/CippAdvanced/
...
│   ├── GenericFunctions.ts           # API helpers: OAuth2 token cache, tenant list cache, cippApiRequest, JSON/endpoint helpers
│   ├── types.ts                      # TypeScript interfaces (ICippCredentials, IAuthToken, ITenant, ITenantCacheEntry)
...
The tenant list (used by the resourceLocator dropdown) is cached in-memory alongside the token cache. Configurable via two credential fields:
- **`enableTenantCache`** (boolean, default `true`) — toggle caching on/off
- **`tenantCacheTtl`** (number, default `30` minutes) — cache TTL in minutes (1–1440)

Cache key is `clientId:tenantId` (same as token cache). Auto-clears on 401 errors and natural TTL expiry. Returns shallow copies to prevent mutation bugs. Each n8n worker process maintains its own cache (no shared state in queue mode).
